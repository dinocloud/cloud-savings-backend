...
ANY_CONSUMER_USER = 3
ANY_PROVIDER_USER = 4
ASSOCIATED_CONSUMER_USER = 5
ASSOCIATED_PROVIDER_USER = 6
...
def validate_admin_user(username, password):
    ...
    if user_dict.get("role").get("id") == USER_ROLES.get("admin") \
            and user_dict.get("status").get("id") == USER_STATUS.get("active"):
        return True
    else:
        return False


def validate_access(basic_auth_string, access_provided):
